...
class ParquetBatchReader:
    ...
    '''
    This class is a fairly thin wrapper around apache arrow for the purposes of reading and batch processing
    parquet files. This constructor allows you to set up a batch reader instance configured directly with a 
    Dataset instance.
    ...
    '''
    @classmethod
    def fromArrowDataSet(cls, dataset:ds.Dataset):
        return cls(dataset)

    '''
    Initialize the batch processer by passing in the location of the dataset
    we wish to process. This is simple constructor which sets up some reasonable
    ...
    '''   
    @classmethod
    def fromFilePath(cls, parquet_path:str):
        return cls(ds.dataset(parquet_path))

    '''
    This is a wrapper function arround Dataset.to_batches(**kwargs) and accepts the same set of arguments as defined
    in the apache arrow docs: https://arrow.apache.org/docs/python/generated/pyarrow.dataset.Dataset.html#pyarrow.dataset.Dataset.to_batches.
    It wraps the call to to_batches(**kwargs). We read each row in each batch and yield the results one at a time allowing
    us to somewhat lazily process records from the underlying parquet_file. Note that I say somewhat as the default batch_readahead
    is 16 batches!
    ...
    '''
    def get_rows_with_args(self, **kwargs) -> Generator:
        ...
        for dataset_batch in self.dataset.to_batches(**kwargs):
            ...
            indices = ((row, col) for row in range(0, num_rows) for col in range(0, num_cols))
            ...
            for row, col in indices:
                row_values.append(dataset_batch[col][row])
    ...
    the underlying Dataset with the selected columns. It is configured as follows:
        ...
        Returns:
            (Generator): A generator where each element represents 1 row from the underlying parquet source
    '''
    def get_rows(self, columns:list[str]) -> Generator:
